...
For every held-out recipient episode this evaluator chooses a different donor
episode with the exact same task.  It evaluates paired observations whose RGB,
robot state, language, target actions, diffusion noise, and inference frame are
identical.  The only intervention is replacing ``depth_gray_view`` or
``surface_normals_view`` with the donor episode's geometry at approximately the
same normalized episode progress.

Dataset files are read-only.  Geometry replacement happens only in memory.
Positive shuffled-minus-intact error means correctly aligned geometry helped
...
CONDITIONS = ("intact", "shuffled_geometry")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare paired intact and same-task shuffled geometry on a held-out "
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of intact/shuffled pairs per inference call (model batch is twice this).",
    )
...
def phase_matched_frame(
    ...
    if target_length <= 0 or donor_length <= 0:
    ...
    if target_length == 1 or donor_length == 1:
        return 0
    mapped = int(np.rint(target_frame * (donor_length - 1) / (target_length - 1)))
...
def replace_geometry(
    flat: dict[str, Any], geometry_key: str, donor_frame: Any
) -> dict[str, Any]:
    ...
    donor = np.asarray([donor_frame])
    target = np.asarray(flat[column])
    if donor.shape != target.shape:
        raise ValueError(
            f"Donor {geometry_key} shape {donor.shape} does not match target {target.shape}"
        )
    if donor.dtype != target.dtype:
        donor = donor.astype(target.dtype, copy=False)
    replaced = dict(flat)
    replaced[column] = donor
...
def _paired_observations(
    *,
    ...
    donor_value = donor_trajectory[f"video.{geometry_key}"].iloc[donor_frame]
    if donor_value is None:
        raise ValueError(
            f"Donor geometry was not decoded for frame {donor_frame} of donor trajectory"
        )
    shuffled = replace_geometry(flat, geometry_key, donor_value)
...
def _prediction_change(
    intact: np.ndarray, shuffled: np.ndarray, indices: np.ndarray
) -> dict[str, float]:
    difference = np.abs(shuffled[:, indices] - intact[:, indices])
    return {
...
def _evaluate_checkpoint(
    *,
    ...
    donor_loader: LeRobotEpisodeLoader,
    records: list[EpisodeRecord],
    ...
    for recipient_number, recipient in enumerate(selected_records, start=1):
        ...
        for repeat, assignments in sorted(assignments_by_repeat.items()):
            assignment = assignments[recipient.loader_position]
            donor_record = record_by_position[assignment.donor_loader_position]
            donor_frames = {
                anchor: phase_matched_frame(
                    anchor, recipient.length, donor_record.length
                )
            ...
            for batch_start in range(0, len(anchors), pair_batch_size):
                ...
                for anchor in batch_anchors:
                    intact_observation, shuffled_observation = _paired_observations(
                        target_trajectory=target_trajectory,
                        target_frame=anchor,
                        donor_trajectory=donor_trajectory,
                        donor_frame=donor_frames[anchor],
                        modality=modality,
                    ...
                    observations.extend((intact_observation, shuffled_observation))
            ...
            shuffled = np.concatenate(episode_shuffled, axis=0)
            ...
            for group_name, indices in groups.items():
                ...
                shuffled_metrics = _error_metrics(
                    shuffled[:, indices], expert[:, indices]
                )
                for condition, metrics in (
                    ("intact", intact_metrics),
                    ("shuffled_geometry", shuffled_metrics),
                ):
                ...
                episode_pair_rows.append(
                    {
                        **base,
                        ...
                        "shuffled_sum_absolute_error": shuffled_metrics[
                            "sum_absolute_error"
                        ],
                        "intact_sum_squared_error": intact_metrics["sum_squared_error"],
                        "shuffled_sum_squared_error": shuffled_metrics[
                            "sum_squared_error"
                        ],
                        "intact_mae": intact_metrics["mae"],
                        "shuffled_mae": shuffled_metrics["mae"],
                        "delta_mae_shuffled_minus_intact": (
                            shuffled_metrics["mae"] - intact_metrics["mae"]
                        ),
                        "intact_mse": intact_metrics["mse"],
                        "shuffled_mse": shuffled_metrics["mse"],
                        "delta_mse_shuffled_minus_intact": (
                            shuffled_metrics["mse"] - intact_metrics["mse"]
                        ),
                        **_prediction_change(intact, shuffled, indices),
                    }
                )
            ...
            raw["shuffled_geometry"].append(shuffled.astype(np.float32))
            raw["episode_index"].append(
                np.full(row_count, recipient.episode_index, dtype=np.int32)
            )
            raw["donor_episode_index"].append(
                np.full(row_count, assignment.donor_episode_index, dtype=np.int32)
            )
        ...
        LOGGER.info(
            "checkpoint %d | %s: recipient %d/%d (episode=%d, task=%s)",
...
def summarize_joints(
    ...
    for joint_index, joint in enumerate(arrays["action_labels"].tolist()):
        ...
        shuffled = _error_metrics(
            arrays["shuffled_geometry"][:, index], arrays["expert"][:, index]
        )
        rows.append(
            {
                "checkpoint_step": checkpoint_step_value,
                ...
                "delta_mae_shuffled_minus_intact": shuffled["mae"] - intact["mae"],
                "intact_mse": intact["mse"],
                "shuffled_mse": shuffled["mse"],
                "delta_mse_shuffled_minus_intact": shuffled["mse"] - intact["mse"],
                **_prediction_change(
                    arrays["intact"], arrays["shuffled_geometry"], index
                ),
            }
        )
    return pd.DataFrame(rows)
    ...
    for scope, scope_value, scoped in scopes:
        for group_name, group in scoped.groupby("group", sort=True):
            per_episode = (
                ...
                .agg(
                    delta_mae=("delta_mae_shuffled_minus_intact", "mean"),
                    delta_mse=("delta_mse_shuffled_minus_intact", "mean"),
            ...
            low, high = _stratified_task_bootstrap_ci(
                group,
                value_column="delta_mae_shuffled_minus_intact",
            ...
            rows.append(
                {
                    ...
                    "micro_shuffled_mae": float(
                        group["shuffled_sum_absolute_error"].sum() / sample_count
                    ),
                    "micro_delta_mae_shuffled_minus_intact": float(
                        (
                            group["shuffled_sum_absolute_error"].sum()
                    ...
                    "micro_shuffled_mse": float(
                        group["shuffled_sum_squared_error"].sum() / sample_count
                    ),
                    "micro_delta_mse_shuffled_minus_intact": float(
                        (
                            group["shuffled_sum_squared_error"].sum()
                    ...
                    "episode_win_rate_shuffled_worse": float(
                ...
                rows.append(
                    {
                        ...
                        "recipient_progress": (
                            anchor / (recipient.length - 1)
                            if recipient.length > 1
                            else 0.0
                        ),
                        "donor_episode_index": assignment.donor_episode_index,
                        "donor_length": assignment.donor_length,
                        "donor_frame": phase_matched_frame(
                            anchor, recipient.length, assignment.donor_length
                        ),
                    }
                )
    return rows
    ...
    *,
    ...
    return {
        ...
        "intervention": {
            ...
            "frame_rule": "round(target_progress * (donor_length - 1))",
            "caveat": "normalized episode progress is approximate, not semantic phase alignment",
        },
        "interpretation": (
            "Prediction change measures geometry sensitivity. Positive shuffled-minus-intact "
            "error measures benefit from correct alignment in open-loop command prediction; "
            "neither quantity proves closed-loop task success."
        ),
        "arguments": arguments,
        ...
        for target_number, target in enumerate(targets, start=1):
            ...
            if canonical_signature is None:
                ...
                _write_csv(
                    ...
                    _donor_manifest(assignments_by_repeat),
                )
                _write_csv(
                    args.output_dir / "frame_mapping.csv",
                    _frame_mapping(
                        selected_records, assignments_by_repeat, args.execution_horizon
                    ),
                )
